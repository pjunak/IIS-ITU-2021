...
/** source: D:\Data\ITnetwork\PHP\Nette\nette-check\sandbox\app/templates/Homepage/default.latte */
final class Templatec74d57e91d extends Latte\Runtime\Template
{
	protected const BLOCKS = [
		['content' => 'blockContent', 'title' => 'blockTitle', 'scripts' => 'blockScripts', 'head' => 'blockHead'],
	];
...
';
		$this->renderBlock('scripts', get_defined_vars()) /* line 53 */;
		echo '


';
		$this->renderBlock('head', get_defined_vars()) /* line 69 */;
		echo "\n";
		return get_defined_vars();
	}
...
<div id="content">
	...
	<section id="template">
		<h2>This page template located at <span>';
		echo LR\Filters::escapeHtmlText(strstr($this->getName(), 'app')) /* line 34 */;
		echo '</span></h2>

		<pre><code class="jush">';
		echo LR\Filters::escapeHtmlText(($this->filters->replacere)(file_get_contents($this->getName()), '#[\w+/]{60,}#', '…')) /* line 36 */;
		echo '</code></pre>
	</section>

	<section id="layout">
		<h2>Layout template located at <span>';
		echo LR\Filters::escapeHtmlText(strstr($this->getParentName(), 'app')) /* line 40 */;
		echo '</span></h2>

		<pre><code class="jush">';
		echo LR\Filters::escapeHtmlText(file_get_contents($this->getParentName())) /* line 42 */;
		echo '</code></pre>
	</section>

	<section id="presenter">
		<h2>Current presenter located at <span>';
		echo LR\Filters::escapeHtmlText(strstr($presenter->getReflection()->getFileName(), 'app')) /* line 46 */;
		echo '</span></h2>

		<pre><code class="jush-php">';
		echo LR\Filters::escapeHtmlText(file_get_contents($presenter->getReflection()->getFileName())) /* line 48 */;
		echo '</code></pre>
	</section>
</div>
';
...
';
	...
	/** {block scripts} on line 53 */
	public function blockScripts(array $ʟ_args): void
	{
		extract($this->params);
		extract($ʟ_args);
		unset($ʟ_args);
		$this->renderBlockParent($ʟ_nm = 'scripts', get_defined_vars()) /* line 54 */;
		echo '<script src="https://files.nette.org/sandbox/jush.js"></script>
<script>
...
';
	...
	body { font: 14px/1.65 Verdana, "Geneva CE", lucida, sans-serif; background: #3484d2; color: #333; margin-top: 40px; }
